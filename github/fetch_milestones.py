...
def fetch():
    ...
    r = requests.get('https://api.github.com/orgs/opendataservices/repos',auth=HTTPBasicAuth(user,password))
    print (r.text)
    with open('data/github/repo.json', 'w', encoding='utf8') as fp:
        fp.write(r.text)
    for repo in json.loads(r.text):
        ...
        with open('data/github/milestones/{0}.json'.format(repo['name']), 'w', encoding='utf8') as fp:
        ...
        with open('data/github/issues_no_milestone/{0}.json'.format(repo['name']), 'w', encoding='utf8') as fp:
            fp.write(issues_no_milestone_request.text)
        
        open_issues = repo['open_issues']
        if open_issues == 0:
            with open('data/github/no_open_issues/{0}.json'.format(repo['name']), 'w', encoding='utf8') as fp:
                fp.write('0')
        ...
        with open('data/github/milestones_closed/{0}.json'.format(repo['name']), 'w', encoding='utf8') as fp:
